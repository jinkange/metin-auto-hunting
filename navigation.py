...
def astar(nav: NavMap, start, goal):
    if start == goal:
        return [start]
    if not nav.free(*goal):
        return []

    def h(p):
        return float(cheb(p, goal))
...
def make_sweep_goals(w, h, stride=2):
    ...
    for y in range(0, h, stride):
        xs = range(0, w, stride) if (y // stride) % 2 == 0 else range(w - 1, -1, -stride)
        for x in xs:
            goals.append((x, y))
    return goals
...
class Navigator:
    ...
    def set_stride(self, stride):
        self.stride = stride
        self.goals = make_sweep_goals(self.nav.w, self.nav.h, stride)
        self.goal_i = 0
        self.path = []
        self.path_i = 0
    ...
    def plan(self, cur):
        goal = self.next_goal()
        self.path = astar(self.nav, cur, goal)
        self.path_i = 0
